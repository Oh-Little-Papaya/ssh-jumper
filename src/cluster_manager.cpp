...
int createTcpListener(const std::string& bindHost, int bindPort,
  ...
  for (auto* ai = result; ai != nullptr; ai = ai->ai_next) {
    ...
    if (getsockname(listenFd, reinterpret_cast<struct sockaddr*>(&boundAddr),
                    &boundLen) == 0) {
      if (boundAddr.ss_family == AF_INET) {
        const auto* v4 =
            reinterpret_cast<const struct sockaddr_in*>(&boundAddr);
        if (actualPort) {
          *actualPort = ntohs(v4->sin_port);
        }
      } else if (boundAddr.ss_family == AF_INET6) {
        const auto* v6 =
            reinterpret_cast<const struct sockaddr_in6*>(&boundAddr);
        if (actualPort) {
...
bool sendAllWithTimeout(int fd, const uint8_t* data, size_t len,
                        int timeoutMs) {
  if (fd < 0 || !data) {
    return false;
  }
...
int connectTcpWithTimeout(const std::string& host, int port, int timeoutMs) {
  ...
  if (gaiRet != 0 || !result) {
    LOG_ERROR("Failed to resolve address " + host + ":" + service + " - " +
              std::string(gai_strerror(gaiRet)));
    return -1;
  }
...
void bridgeSocketPair(int leftFd, int rightFd) {
  ...
  while (true) {
    ...
    if (pfds[0].revents & POLLIN) {
      ...
      if (n <= 0 ||
          !sendAllWithTimeout(rightFd, reinterpret_cast<const uint8_t*>(buffer),
    ...
    if (pfds[1].revents & POLLIN) {
      ...
      if (n <= 0 ||
          !sendAllWithTimeout(leftFd, reinterpret_cast<const uint8_t*>(buffer),
                              static_cast<size_t>(n), kSendTimeoutMs)) {
        break;
      }
    }
...
bool AgentMessage::deserialize(const uint8_t* data, size_t len,
                               AgentMessage& msg) {
  if (len < 9) {
    return false;
  }
...
AgentMessage AgentMessage::create(AgentMessageType type,
...
AgentConnection::AgentConnection(int fd, ClusterManager* manager)
    : fd_(fd), manager_(manager) {}

AgentConnection::~AgentConnection() { close(); }
...
int AgentConnection::onRead() {
  ...
  while (readBuffer_.readableBytes() >= 9) {
    ...
    if (!AgentMessage::deserialize(data, frameSize, msg)) {
      LOG_WARN("Failed to deserialize agent message on fd " +
               std::to_string(fd_));
      return -1;
    }
...
int AgentConnection::onWrite() {
...

  {
    ...
    while (writeBuffer_.readableBytes() > 0) {
      ssize_t n = send(fd_, writeBuffer_.readableData(),
                       writeBuffer_.readableBytes(), MSG_NOSIGNAL);
      if (n > 0) {
...
void AgentConnection::handleRegister(const std::string& payload) {
  ...
  if (registerJson.contains("services") &&
      registerJson["services"].is_array()) {
...
void AgentConnection::handleCommand(const std::string& payload) {
  ...
  if (op == "get_user" || op == "create_user" || op == "update_user" ||
      op == "delete_user") {
    ...
    if (op == "create_user") {
      ...
      const std::string passwordHash =
          trimString(req.value("passwordHash", ""));
    ...
    if (op == "update_user") {
      ...
      const bool updated =
          configManager.updateUser(username, [&](UserAuthInfo& user) {
            ...
            if (req.contains("passwordHash")) {
              const std::string passwordHashValue =
                  trimString(req.value("passwordHash", ""));
  ...
  if (op == "get_node" || op == "create_node" || op == "update_node" ||
      op == "delete_node") {
...
bool ClusterManager::initialize(const std::string& listenAddr, int port,
  ...
  LOG_INFO("ClusterManager initialized on " + listenAddr + ":" +
           std::to_string(port));
  return true;
}

void ClusterManager::setReverseTunnelOptions(int portStart, int portEnd,
                                             int retries, int acceptTimeoutMs) {
  if (portStart <= 0 || portStart > 65535 || portEnd <= 0 || portEnd > 65535 ||
      portStart > portEnd) {
    LOG_WARN(
        "Invalid reverse tunnel port range provided, fallback to defaults");
  ...
  if (acceptTimeoutMs <= 0) {
    LOG_WARN(
        "Invalid reverse tunnel accept timeout provided, fallback to default");
  ...
  LOG_INFO("Reverse tunnel options updated: ports=" +
           ...
           std::to_string(reverseTunnelPortEnd_) + ", retries=" +
           std::to_string(reverseTunnelRetries_) + ", acceptTimeoutMs=" +
           std::to_string(reverseTunnelAcceptTimeoutMs_));
}
...
bool ClusterManager::start() {
  ...
  timerThread_->start();
  timerThread_->scheduleRepeat(std::chrono::seconds(30),
                               [this]() { this->checkAgentHealth(); });
...
bool ClusterManager::registerAgent(const AgentInfo& info, int controlFd) {
  ...
  if (!sharedToken_.empty() && timingSafeEqual(info.authToken, sharedToken_)) {
    tokenValid = true;
    LOG_INFO("Agent " + info.agentId +
             " authenticated by shared cluster token");
  }

  if (!tokenValid && configIt != agentConfigs_.end()) {
    // 使用预配置的完整配置
    if (timingSafeEqual(configIt->second.token, info.authToken)) {
      tokenValid = true;
      // 使用预配置的 IP（关键修改）
      ...
      LOG_INFO("Using pre-configured IP for agent " + info.agentId + ": " +
               effectiveIp);
    }
  } else if (!tokenValid && tokenIt != agentTokens_.end()) {
    // 简单格式，只验证 token
    if (timingSafeEqual(tokenIt->second, info.authToken)) {
      tokenValid = true;
      LOG_WARN("Agent " + info.agentId +
               " using simple token config, IP from agent: " + effectiveIp);
    }
  }
  ...
  // 检查 IP 是否有效（不能是回环地址）
  if (effectiveIp == "127.0.0.1" || effectiveIp == "127.0.1.1" ||
      effectiveIp.empty()) {
    LOG_ERROR("Invalid IP address for agent " + info.agentId + ": " +
              effectiveIp);
  ...
  if (existingIt != agents_.end() && existingIt->second) {
    ...
    if (existingIt->second->controlFd >= 0 &&
        existingIt->second->controlFd != controlFd) {
      pendingConnections_.erase(existingIt->second->controlFd);
      LOG_WARN("Agent " + info.agentId +
               " re-registered with new control connection, old fd=" +
               std::to_string(existingIt->second->controlFd) +
               ", new fd=" + std::to_string(controlFd));
    }
  }
  ...
  LOG_INFO("Agent registered: " + info.agentId + " (" + agent->hostname +
           ") at " + agent->ipAddress);
  return true;
}
...
void ClusterManager::unregisterAgentByConnection(const std::string& agentId,
                                                 int controlFd) {
  std::lock_guard<std::mutex> lock(mutex_);
...
std::shared_ptr<AgentInfo> ClusterManager::getAgent(
...
std::shared_ptr<AgentInfo> ClusterManager::findAgentByHostname(
...
bool ClusterManager::verifyAgentToken(const std::string& agentId,
                                      const std::string& token) {
  if (token.empty()) {
    return false;
  }
...
std::optional<AgentTokenConfig> ClusterManager::getConfiguredAgent(
...
int ClusterManager::establishForwardConnection(const std::string& agentId,
                                               int targetPort) {
  ...
  if (controlConn) {
    ...
    const int maxAttempts =
        reverseTunnelRetries_ > 0 ? reverseTunnelRetries_ : 1;
    ...
    for (int attempt = 1; attempt <= maxAttempts; ++attempt) {
      ...
      if (listenFd < 0) {
        ...
        LOG_WARN("Reverse tunnel attempt " + std::to_string(attempt) + "/" +
                 ...
                 " failed to bind callback listener for agent " + agentId +
                 " on port " + std::to_string(preferredPort));
        continue;
      }
      ...
      if (!controlConn->sendMessage(request)) {
        ...
        LOG_WARN("Reverse tunnel attempt " + std::to_string(attempt) + "/" +
                 std::to_string(maxAttempts) +
                 " failed to send FORWARD_REQUEST for agent " + agentId +
                 ", requestId=" + requestId +
      ...
      if (pollRet > 0 && (pfd.revents & POLLIN)) {
        ...
        LOG_WARN("Reverse tunnel attempt " + std::to_string(attempt) + "/" +
                 std::to_string(maxAttempts) + " accept failed for agent " +
        ...
        LOG_WARN("Reverse tunnel attempt " + std::to_string(attempt) + "/" +
                 std::to_string(maxAttempts) + " timed out for agent " +
                 agentId + ", requestId=" + requestId + ", callbackPort=" +
                 std::to_string(connectBackPort) + ", timeoutMs=" +
                 std::to_string(reverseTunnelAcceptTimeoutMs_));
      } else {
        lastReverseFailure = "accept_poll_failed";
        LOG_WARN("Reverse tunnel attempt " + std::to_string(attempt) + "/" +
                 std::to_string(maxAttempts) + " poll failed for agent " +
                 agentId + ", requestId=" + requestId +
                 ", callbackPort=" + std::to_string(connectBackPort) +
                 ", error=" + std::string(strerror(errno)));
      }
...
void ClusterManager::acceptConnection() {
  while (true) {
    ...
    int clientFd =
        accept4(listenFd_, reinterpret_cast<struct sockaddr*>(&clientAddr),
...
ClusterAgentClient::~ClusterAgentClient() { stop(); }

bool ClusterAgentClient::initialize(const std::string& serverAddr,
                                    int serverPort, const std::string& agentId,
...
void ClusterAgentClient::workerLoop() {
  ...
  while (running_) {
    ...
    if (ret < 0) {
      ...
      LOG_ERROR("Poll error on control connection: " +
...
void ClusterAgentClient::handleForwardRequest(const std::string& payload) {
  ...
  try {
    ...
    LOG_WARN("Invalid forward request payload: not a valid JSON object");
    return;
  }

  if (!req.is_object()) {
    LOG_WARN("Invalid forward request payload: expected JSON object");
    return;
  }

  const std::string requestId = trimString(req.value("requestId", ""));
  std::string targetHost = trimString(req.value("targetHost", ""));
  if (targetHost.empty()) {
    targetHost = "127.0.0.1";
  }
  const std::string connectBackHost =
  ...
  const std::string requestLabel =
      requestId.empty() ? generateUUID() : requestId;
  ...
  submitForwardTask([requestLabel, callbackHost, connectBackPort, targetHost,
    ...
    if (targetFd < 0) {
      LOG_ERROR("Forward request " + requestLabel +
                " failed: cannot connect target " + targetHost + ":" +
                std::to_string(targetPort));
      return;
    }

    int tunnelFd =
        connectTcpWithTimeout(callbackHost, connectBackPort, kConnectTimeoutMs);
    if (tunnelFd < 0) {
      LOG_ERROR("Forward request " + requestLabel +
    ...
    setTcpNoDelay(tunnelFd);
    LOG_INFO("Forward request " + requestLabel + " established: server " +
             callbackHost + ":" + std::to_string(connectBackPort) +
             " <-> target " + targetHost + ":" + std::to_string(targetPort));
...
bool ClusterAgentClient::connectToServer() {
  ...
  if (sockFd_ < 0) {
    LOG_ERROR("Failed to connect to server: " + serverAddr_ + ":" +
  ...
  connected_ = true;
  LOG_INFO("Connected to server: " + serverAddr_ + ":" +
           std::to_string(serverPort_));
  return true;
}
...
bool ClusterAgentClient::receiveMessage(AgentMessage& msg) {
  ...
  // 验证 magic number
  if (msg.magic != MAGIC_NUMBER) {
    LOG_ERROR("Invalid magic number in message: 0x" +
              std::to_string(msg.magic));
    return false;
  }
  ...
  if (msg.length > 0) {
    ...
    while (totalRead < static_cast<ssize_t>(msg.length)) {
      ...
      ssize_t n =
          recv(sockFd_, &msg.payload[totalRead], msg.length - totalRead, 0);
      if (n <= 0) {
        if (n == 0) {
          LOG_WARN("Connection closed by server during payload read");
        } else if (errno != EAGAIN && errno != EWOULDBLOCK) {
          LOG_ERROR("Recv error during payload: " +
                    std::string(strerror(errno)));
        }
        connected_ = false;
        return false;
...
std::string ClusterAgentClient::getLocalIpAddress() {
  ...
  if (testSock >= 0) {
    ...
    if (connect(testSock, (struct sockaddr*)&remoteAddr, sizeof(remoteAddr)) ==
        0) {
      struct sockaddr_in localAddr;
      socklen_t addrLen = sizeof(localAddr);
