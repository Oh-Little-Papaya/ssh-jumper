...
SessionRecorder::~SessionRecorder() { stopRecording(); }
...
bool SessionRecorder::startRecording(const std::string& sessionPath,
  ...
  LOG_INFO("Session recording started: " + sessionId_ + " -> " +
           recordFilePath_);
  return true;
}
...
void SessionRecorder::record(Direction direction, const char* data,
                             size_t len) {
...
void SessionRecorder::writeHeader() {
  ...
  // Start timestamp (milliseconds since UNIX epoch, wall clock)
  auto epoch = startWallTime_.time_since_epoch();
  uint64_t timestamp =
      std::chrono::duration_cast<std::chrono::milliseconds>(epoch).count();
  recordFile_.write(reinterpret_cast<const char*>(&timestamp),
                    sizeof(timestamp));

  recordFile_.flush();
}

void SessionRecorder::writeRecord(Direction direction, const char* data,
                                  size_t len) {
  // 记录格式:
  ...
  // 写入时间偏移
  recordFile_.write(reinterpret_cast<const char*>(&timestampOffset),
                    sizeof(timestampOffset));
  ...
  recordedSize_ +=
      sizeof(timestampOffset) + sizeof(dir) + sizeof(dataLen) + len;
